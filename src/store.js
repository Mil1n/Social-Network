...
export class Store {
  ...
  async createChat(userId, { title, memberIds=[] }) { const chat = { id: randomUUID(), title: title || 'New chat', memberIds: [...new Set([userId, ...memberIds])], createdBy: userId, createdAt: new Date().toISOString(), lastReadBy: { [userId]: new Date().toISOString() } }; this.db.chats.push(chat); await this.save(); return chat; }
